...
# 	assert len(trainloader) > 0
		# Load labels
	for batch_num, (data_image, data_pcl, label_) in enumerate(trainloader):
...
# 	train_summary_writer = tf.summary.create_file_writer(train_log_dir)
	...
	for epoch in range(numEpochs):	
		avg_loss = 0
		iters = 0
		for batch_num, (data_image, data_pcl, label_) in enumerate(trainloader):  # TODO Setup to feed the Labels from train_loader
